...
base_url <- paste0("http://www.cbssports.com/fantasy/hockey/stats/sortable/points/")
end_url <- paste0("/standard/stats/2017/ytd")
...
goalieCategoryId <- "G"
centerCategoryId <- "C"
rightWingCategoryId <- "RW"
leftWingCategoryId <- "LW"
defenseCategoryId <- "D"

#for paging
showAll <- "?&print_rows=9999"

cbs_goalie_urls <- paste0(base_url, "", goalieCategoryId, "", end_url, "", showAll)
# cbs_defense_urls <- paste0(base_url, "&slotCategoryId=", defenseCategoryId , "&startIndex=", espn_pages)
# cbs_rightwing_urls <- paste0(base_url, "&slotCategoryId=", forwardCategoryId , "&startIndex=", espn_pages)
# cbs_leftwing_urls <- paste0(base_url, "&slotCategoryId=", forwardCategoryId , "&startIndex=", espn_pages)
# 
# 
...
#goaliesList <- lapply(cbs_goalie_urls, function(x){
#    readHTMLTable(x, skip.rows = c(1), as.data.frame=TRUE, stringsAsFactors=FALSE)$playertable_0
#})

goaliesList <- readHTMLTable(getURL(cbs_goalie_urls)) 
